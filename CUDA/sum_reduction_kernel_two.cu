...
__global__ void sumReduction(int *v, int *v_r)
{
    ...
    // iterate of log base 2
    for (int s = 1; s < blockDim.x; s *= 2)
    {
        // reduce the threads performing work by half previous the previous iteration each cycle
        if (threadIdx.x % (2 * s) == 0) // wrap divergence only few threads are being utitlised rest are sitting idle
        {
    ...
    // let the thread 0 of this block write its result in main memory
